...
export function startGateway(options) {
    ...
    const verifySession = (value) => {
        ...
        if (!safeEqualHex(sig, hmac(payload)))
            return false;
        try {
    ...
    const authKind = (parsed) => {
        ...
        if (session !== null && verifySession(session[1] ?? ''))
            return 'cookie';
        const bearer = parsed.map.authorization ?? '';
        if (bearer.startsWith('Bearer ') && bearer.slice(7) === token)
            return 'bearer';
        try {
            if (new URL(parsed.target, 'http://gateway.local').searchParams.get('token') === token)
                return 'query';
        }
        catch { /* fall through */ }
        return 'none';
    ...
    const registerFailure = (ip) => {
        ...
        if (attempts.size > 512) {
            for (const [key, value] of attempts) {
                if (value.fails === 0 && value.blockedUntil === undefined)
                    attempts.delete(key);
            }
        }
    };
