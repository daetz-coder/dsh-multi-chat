...
- **手机/远程访问**：工具栏「手机访问」按钮读取本机局域网 IP；官方 CLI 出于安全禁止 `--host 0.0.0.0`，因此跨设备访问需在实例前挂一个**带令牌认证的网关**（`dsh-multi-wall gateway`），按钮会给出对应提示。
...
- id: ui-multi-wall
  ...
  config:
    ...
    scanTo: 3110
    ports: []        # 设置后不再自动扫描
    publicUrl: ''    # 可选：对外网关地址（如 https://host:8443），/multi/api/link 会直接返回它并标记可达
```
